...
function addServeOptions(command: Command): Command {
  return command
    ...
    .option(
      '--voicevox-image <image>',
      'VOICEVOX Docker image',
      'voicevox/voicevox_engine:cpu-latest'
    );
}

export function serveCommand(): Command {
  return addServeOptions(
    new Command('serve').description('Start the VOICEVOX Engine and, when needed, Ollama')
...
export function servicesCommand(): Command {
  ...
  command.addCommand(
    addServeOptions(new Command('start').description('Start required services')).action(
      serveServices
    )
  );
  command.addCommand(
    new Command('status')
      .description('Check required service availability')
      .action(showServiceStatus)
  );
  command.addCommand(
    new Command('stop').description('Stop the APVG-managed VOICEVOX container').action(stopServices)
  );
  return command;
}
