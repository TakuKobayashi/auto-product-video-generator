...
export class CliRecorder implements PlatformRecorder {
  ...
  private async ensureContainer(): Promise<void> {
    if (this.containerName) return;
    if (!this.context.rootDir) throw new Error('CLI recording requires a resolved source directory.');
  ...
  private async ensureSetup(): Promise<void> {
    ...
    for (const step of this.context.setupSteps) {
      ...
      if (result.code !== 0) throw new Error(`CLI setup failed (${result.code}): ${step.command}\n${result.stderr}`);
    }
...
async function appendTerminal(page: import('playwright').Page, text: string, delay: number): Promise<void> {
  await page.evaluate(async ({ value, charDelay }) => {
...
function runProcess(command: string, args: string[], rejectOnFailure = true): Promise<{ code: number; stdout: string; stderr: string }> {
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.on('close', (code) => {
      ...
      if (rejectOnFailure && result.code !== 0) {
        rejectRun(new Error(`${command} ${args.join(' ')} failed (${result.code}): ${stderr.trim()}`));
      } else {
        resolveRun(result);
      }
    });
