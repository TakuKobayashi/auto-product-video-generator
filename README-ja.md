...
#    ※ まっさらな状態のクローン直後でもこのコマンド1つでOKです（内部で pnpm install も実行されます）
pnpm run setup
...
`pnpm run setup` を実行すると [`Task`](https://taskfile.dev)（devDependencyの `@go-task/cli`）も
一緒にインストールされるため、以降は読みやすい `task <コマンド名>` という形式も使えます
（詳細は [`Taskfile.yml`](./Taskfile.yml) を参照。各タスクには日本語の説明が付いており、
`task --list` で一覧表示できます）。どちらの形式も内部的には同じスクリプトを呼び出しているだけなので、
好きな方を使ってください。社内プロキシなどで `@go-task/cli` のバイナリダウンロードがブロックされる
環境では、上記の `pnpm run ...` コマンドがそのまま代替として機能します（Taskは不要です）。

生成された動画は `./output/final.mp4` に出力されます。

> 「そもそもどう動かせばいいかわからない」という場合は、まず `pnpm run doctor` を実行してみてください。
...
「ローカルでの運用がやりづらい」という課題に対応するため、環境構築・サーバー起動をそれぞれ
**コマンド1つ**に集約しています（内部実装は [`Taskfile.yml`](./Taskfile.yml) + `scripts/*.mjs`）。

| やりたいこと | `pnpm run`（常に動作） | `task`（読みやすい名前、任意） |
|---|---|---|
| 環境を一括インストール（ローカルLLM込み） | `pnpm run setup:full` | `task install` |
| 環境を一括インストール（クラウドLLMのみ、Ollama無し） | `pnpm run setup:cloud-only` | `task install WITH_OLLAMA=false` |
| GitHub Actions向けにインストール | `pnpm run setup:ci` | `task install PROFILE=ci` |
| ローカルサーバーを一括起動（VOICEVOX + Ollama） | `pnpm run serve` | `task serve` |
| ローカルサーバーを停止 | `pnpm run serve:stop` | `task stop` |
| 環境に不足がないか診断 | `pnpm run doctor` | `task doctor` |
| インストール→起動→次の手順の案内までまとめて実行 | — | `task quickstart` |

`PROFILE`（`local`がデフォルト / `ci`）と `WITH_OLLAMA`（`true`がデフォルト / `false`）は、
`task` コマンド利用時に自由に組み合わせて指定できます。
...
内部で呼ばれているスクリプトを直接叩くこともできます（動作は完全に同じで、Taskは不要です）。
...
タスクの全一覧は `task --list` で確認できます（各タスクの説明は日本語で書かれています）。

### `install` が実際にやっていること

1. `pnpm install`（Node依存関係のインストール。この際 `ffmpeg-static` / `ffprobe-static` の
   postinstallスクリプトが実行され、OS・CPUアーキテクチャに応じたffmpeg/ffprobeのバイナリが
   自動ダウンロードされ、`node_modules` 配下の適切な場所に配置されます。CLI側は
   `resolveFfmpegPath()` / `resolveFfprobePath()` によりこのバイナリを自動的に検出して使用するため、
   システムに別途ffmpegをインストールしたりPATHを通したりする作業は不要です）
2. `node scripts/install-playwright.mjs`（PlaywrightのChromiumブラウザをダウンロード。
   Linux環境では追加でOS依存パッケージのインストールも試行します）
3. （`WITH_OLLAMA=true` の場合のみ）`node scripts/install-ollama.mjs`
   - Ollama本体が未インストールの場合、OSを自動判定してインストールを試みます
     - Linux: 公式インストールスクリプト（`curl -fsSL https://ollama.com/install.sh | sh`）
     - macOS: Homebrewがあれば `brew install ollama`
     - Windows: wingetがあれば `winget install --id Ollama.Ollama`
     - 自動インストールに失敗した場合は [ollama.com/download](https://ollama.com/download) から
       手動インストールするよう案内を表示します
   - `PROFILE` に応じたモデル（後述）を `ollama pull` します

### `serve` が実際にやっていること

1. VOICEVOX Engineが `http://localhost:50021` で応答するか確認し、応答がなければ
   Dockerコンテナ（`docker run -d --rm --name dvg-voicevox -p 50021:50021 ...`）として起動し、
   ヘルスチェックが通るまで待機します（Dockerが無い場合はインストール方法を案内して終了します）
2. （`WITH_OLLAMA=true` の場合のみ）Ollamaデーモンが `http://localhost:11434` で応答するか確認し、
   応答がなければバックグラウンドで `ollama serve` を起動します。合わせて対象モデルが
   pull済みであることも確認します
...
自動的に選択します（定義は `scripts/lib/model-profiles.mjs` に集約されています）。
...
scripts/          Taskfileから呼び出されるインストール/起動/診断スクリプト（Node製、クロスプラットフォーム）
  ├── install.mjs           環境の一括インストール（`task install` の実体）
  ├── install-playwright.mjs
  ├── install-ollama.mjs    Ollama本体 + モデルのインストール
  ├── serve.mjs              サーバーの一括起動（`task serve` の実体）
  ├── stop.mjs                サーバーの停止
  ├── doctor.mjs               環境診断（`task doctor` の実体）
  └── lib/
      ├── model-profiles.mjs  local/ci向けOllamaモデルの定義（唯一の定義元）
      └── proc.mjs             プロセス起動・ヘルスチェック等の共通処理

Taskfile.yml      go-task定義。コマンド1つでの環境構築・サーバー起動の入り口
```
...
`package.json` には `task` コマンドへの薄いエイリアスも用意しています。
...
| `pnpm run doctor` | `node scripts/doctor.mjs`（`task doctor` と同じ） |
...
2. ffmpegはシステムに手動でインストールしても構いません（自動的に検出されます）
   ...
   ```
3. `task`（`@go-task/cli`）はあくまで利便性のためのレイヤーです。無くても
   `pnpm run setup` / `pnpm run serve` 等がすべて代替として機能します
...
`task serve` / `pnpm run serve` を実行済みか確認してください。Dockerが必要です
...
`task doctor` / `pnpm run doctor` を実行すると、上記も含めた環境全体の状態を
まとめて確認できます。
